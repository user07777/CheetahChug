...
void CDraw::Text(char* text, float x, float y, int orientation, int font, bool bordered, DWORD color, DWORD bcolor)
{
    ...
    case centered:
        ...
        if (pFont != NULL) {
            ...
            //update
            this->AddFont(L"Arial", 20, false, false);
        }
        break;
    case righted:
        if (bordered)
...
void CDraw::AddFont(const wchar_t* Caption, float size, bool bold, bool italic)
{
...
{
    for (int i = 0; i < FontNr; i++) pFont->OnLostDevice();
    for (int i = 0; i < FontNr; i++) pFont->OnResetDevice();
}
